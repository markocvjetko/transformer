...
def evaluate(model: LitGPT, dataloader: DataLoader, device: torch.device):
...
if __name__ == "__main__":
    ...
    transformer = LitGPT(vocab_size=vocab_size, seq_len=seq_len, d_model=d_model, d_ff=d_ff, n_heads=n_heads, N=N).to(
        device
    )
    ...
    dataset_val = TranslationDataset(hf_ds_val, tokenizer_src=tokenizer_src, tokenizer_tgt=tokenizer_tgt, max_len=128)

    # Minimal dataloader
    dataloader_train = DataLoader(dataset_train, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)

    dataloader_val = DataLoader(
        dataset_val, batch_size=batch_size, shuffle=False, collate_fn=collate_fn, drop_last=True
    )
    ...
    for epoch in range(n_epochs):
        total_loss = 0.0
        for _batch_idx, (input_seq, target_seq) in enumerate(dataloader_train):
            input_seq = input_seq.to(device)
