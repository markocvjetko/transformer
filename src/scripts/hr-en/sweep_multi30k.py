...
def objective(trial: optuna.Trial, config):
    ...
    dataset_val = TranslationDataset(
        ds_val, tokenizer_src=tokenizer, tokenizer_tgt=tokenizer, max_len=config["seq_len"]
    )

    # Minimal dataloader
    dataloader_train = DataLoader(
        dataset_train, batch_size=batch_size, shuffle=True, num_workers=16, collate_fn=collate_fn
    )

    dataloader_val = DataLoader(
        dataset_val, batch_size=batch_size, shuffle=False, num_workers=16, collate_fn=collate_fn
    )
