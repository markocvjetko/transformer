...
def main():
    ...
    opus = load_dataset("Helsinki-NLP/opus-100", "en-hr", cache_dir=config["opus"], split="test")
    ...
    for input_seq, target_seq in tqdm(dataloader_test, desc="Evaluating", unit="batch"):
        ...
        output = model.translate_beam_search(input_seq, y = torch.full((input_seq.shape[0], 1), 1, dtype=torch.long).to("cuda"))
